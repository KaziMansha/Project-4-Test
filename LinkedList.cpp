...
LinkedList<ItemType>::LinkedList(const LinkedList<ItemType> &list) : head_{nullptr}, size_{0} {
    if (list.isEmpty() == false) {
        Node<ItemType>* current = list.getHeadPtr();
        int counter = 0;
        while (counter < list.getSize()) {
            insert(current -> getItem(), counter);
            counter ++;
...
//Reverse Copy
template<typename ItemType>
void LinkedList<ItemType>::reverseCopy(const LinkedList<ItemType> &list) {
    LinkedList<ItemType> newList(list);
    Node<ItemType>* temp_ptr = newList.getHeadPtr();
    const int start = 0;
    const int end = newList.getSize();
    while(temp_ptr != nullptr) {
        insert(temp_ptr -> getItem(), start);
...
bool LinkedList<ItemType>::moveItemToTop(const ItemType &item) {
    if (getIndexOf(item) < 1) {
        ...
        Node<ItemType>* temp_ptr = new Node<ItemType>();
        temp_ptr -> setItem(item);
        remove(getIndexOf(item));
        temp_ptr ->setNext(head_);
        head_ = temp_ptr;
        return true;
    }
}
